...
class ContextualReasoningModule(nn.Module):
    ...
    Predicts the modulation vector (query-wise adaptive bias) based on the contextual probe tokens.
    """

    def __init__(self, input_dim: int, nhead: int = 8, num_encoder_layers: int = 2, hidden_dim: int = 512, output_dim: int = 1):
        super().__init__()
    ...
    def forward(self, fused_features: torch.Tensor) -> torch.Tensor:
        ...
        modulation_vector = self.activation(output)
...
@registry.register_model("oacir_adafocal_vector")
class Blip2QformerOacirAdaFocalVector(Blip2Base):
    """
    AdaFocal Vector Variant Model for the OACIR task.
    Features the Context-Aware Attention Modulator (CAAM) to dynamically predict query-wise modulation vector.
    ...
    Usage:
        ...
        >>> model = load_model("oacir_adafocal_vector", "pretrain")
    """
    ...
    def forward(self, samples):
        ...
        if is_highlighting:
            ...
            # Predict Modulation Vector via CRM
            adaptive_bias_vector = self.crm_module(pre_fusion_features)

            # Generate Query-wise Modulated Attention Bias
            batch_patch_mask = bbox_to_patch_mask(reference_bbox, patch_size=self.patch_size, device=reference_image.device)
            attention_bias = (adaptive_bias_vector.unsqueeze(-1) * batch_patch_mask.unsqueeze(1)).unsqueeze(1)
            # Format the bias for proper broadcasting across the M multimodal fusion queries.
    ...
    def inference(self, reference_image_embeds, target_features, modification_text, reference_bbox=None):
        ...
        if is_highlighting:
            return sim_f2t, adaptive_bias_vector
        return sim_f2t
