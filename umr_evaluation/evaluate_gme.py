...
def extract_index_features(dataset, model, batch_size: int, num_workers: int) -> tuple[torch.Tensor, list[str]]:
    ...
    for names, images in tqdm(candidate_dataloader, ncols=140, ascii=True):
        with torch.no_grad():
            embeddings = model.get_image_embeddings(images=images)
...
def compute_oacirr_metrics(relative_val_dataset, model, index_features, index_names, instructions, batch_size: int, num_workers: int, save_results: bool):
    ...
    for ref_names, target_names, mod_texts, ref_images in tqdm(relative_dataloader, ncols=140, ascii=True):
        batch_instructions = [random.choice(instructions) for _ in range(len(mod_texts))]

        with torch.no_grad():
            query_embeddings = model.get_fused_embeddings(images=ref_images, texts=mod_texts, instruction=batch_instructions)
...
def compute_fiq_metrics(relative_val_dataset, model, index_features, index_names, instructions, batch_size: int, num_workers: int, save_results: bool):
    ...
    for ref_names, target_names, mod_texts, ref_images in tqdm(relative_dataloader, ncols=140, ascii=True):
        ...
        batch_instructions = [random.choice(instructions) for _ in range(len(processed_mod_texts))]

        with torch.no_grad():
            query_embeddings = model.get_fused_embeddings(images=ref_images, texts=processed_mod_texts, instruction=batch_instructions)
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser("Validate the GME model on the OACIRR benchmark and standard CIR datasets")
    ...
    parser.add_argument("--model-path", type=str, default="Alibaba-NLP/gme-Qwen2-VL-7B-Instruct", help="Path to the local GME model or HF model ID")
    parser.add_argument("--batch-size", type=int, default=8, help="The batch size during the validation")
    ...
    parser.add_argument("--results-dir", type=str, default='./umr_results/GME-7B', help="Directory to save the results")
    parser.add_argument("--results-name", type=str, default='validation_metrics', help="File name prefix to save the result")

    args = parser.parse_args()


    model = AutoModel.from_pretrained(args.model_path, trust_remote_code=True, torch_dtype=torch.float16).to(device).eval()
